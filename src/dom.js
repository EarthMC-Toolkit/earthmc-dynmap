...
const waitForStableViewport = () => new Promise(resolve => {
	...
	if (!pane?.classList.contains('leaflet-pan-anim')) return resolve() // no pane element or already stable

	// Keep checking for removal of 'leaflet-pan-anim' which indicates the map stopped panning.
	const observer = new MutationObserver(() => {
